      ...
      <div className="container mx-auto px-4 py-4 md:py-5 flex justify-between items-center bg-slate-950">
        ...
        <nav className="hidden md:flex items-center space-x-8">
          {navLinks.map(link => link.submenu ? <div key={link.name} className="relative group">
                <Link to={link.path} className={`font-medium hover:text-amber-400 transition-all duration-300 flex items-center ${location.pathname.includes(link.path) ? 'text-amber-400 border-b-2 border-amber-400' : 'text-gray-300'}`}>
                  {link.name}
                ...
                <div className="absolute left-0 mt-1 w-48 bg-[#0F1524] shadow-lg rounded-md overflow-hidden z-10 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 border border-gray-700">
                  {link.submenu.map(subItem => <Link key={subItem.name} to={subItem.path} className="block px-4 py-3 text-gray-300 hover:bg-gradient-to-r hover:from-amber-500 hover:to-villa-terracotta hover:text-white transition-all duration-300">
                      {subItem.name}
                    </Link>)}
                </div>
              </div> : <Link key={link.name} to={link.path} className={`font-medium hover:text-amber-400 transition-colors ${isActive(link.path) ? 'text-amber-400 border-b-2 border-amber-400' : 'text-gray-300'}`}>
                {link.name}
              </Link>)}
      ...
      {mobileMenuOpen && <div className="md:hidden absolute top-full left-0 right-0 bg-[#0F1524] shadow-lg border-t border-gray-800 animate-fade-in z-50">
          <div className="container mx-auto px-4 py-3 flex flex-col space-y-3">
            {navLinks.map(link => link.submenu ? <div key={link.name} className="py-2">
                  ...
                  <div className="pl-6">
                    {link.submenu.map(subItem => <Link key={subItem.name} to={subItem.path} className={`py-2 px-3 block ${isActive(subItem.path) ? 'text-amber-400 font-medium' : 'text-gray-300 hover:text-amber-400'} transition-colors`} onClick={() => setMobileMenuOpen(false)}>
                        {subItem.name}
                      </Link>)}
                  </div>
                </div> : <Link key={link.name} to={link.path} className={`py-2 px-3 block ${isActive(link.path) ? 'text-amber-400 font-medium' : 'text-gray-300 hover:text-amber-400'} transition-colors`} onClick={() => setMobileMenuOpen(false)}>
                  {link.name}
                </Link>)}
